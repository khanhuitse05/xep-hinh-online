jQuery(document).ready(function($) {
	...
	$('.ois_wrapper form').submit(function(e) {
		...
		while (unfoundWrapper) {
			...
			if (maybeWrapper.attr('class') == 'ois_wrapper') {
				unfoundWrapper = false;
				var id = $(maybeWrapper).attr('data');
			}
		}
		if ($.inArray(id, submitted) > -1) {} else {
			submitted.push(id);
			var ois_post_id = $('.ois_wrapper').attr('rel');
			var ois_skin_id = id;
			var ois_service = $(this).attr('service');
			...
			setTimeout(function() {
				$.ajax({
					...
					success: function(data) {
						if (ois_service == 'mailchimp' || ois_service == 'feedburner') {
							if (data != 0 && data != 'no_redirect') {
